...
def write_or_merge_futures_csv(csv_path: str, new_frames_by_symbol: dict) -> pd.DataFrame | None:
    ...
    "시도했지만 실패해서 빈 DataFrame만 나온 심볼"은 여기서 절대 기존 데이터를
    지우면 안 된다 — 2026-08-31 실제로 이 조건을 놓쳐서, NQ 캡처가 이번 실행에서
    실패했다는 이유만으로 CSV에서 NQ가 통째로 사라지는 사고가 있었다. "이번에
    시도한 심볼 집합"이 아니라 "이번에 실제로 새 데이터를 얻은 심볼 집합"만
    기준으로 기존 행을 걸러내야 한다."""
    existing = None
    ...
    non_empty = {sym: df for sym, df in new_frames_by_symbol.items() if df is not None and not df.empty}
    failed_symbols = sorted(set(new_frames_by_symbol.keys()) - set(non_empty.keys()))
    if failed_symbols:
        print(f"  [정보] 이번 캡처에서 데이터를 못 얻은 심볼: {failed_symbols} "
              f"→ 기존 CSV의 해당 심볼 데이터를 그대로 보존합니다(지우지 않음).")

    frames = []
    captured_symbols = set(non_empty.keys())  # 실패한 심볼은 여기 안 들어감 -> 기존 행 보존됨
...
async def capture_greeks_one(symbol: str, cfg: dict, async_playwright) -> pd.DataFrame:
    ...
    async with async_playwright() as p:
        browser = await p.chromium.launch(
            headless=True,
            # 몇몇 기본 자동화 탐지 신호(자동화 감지용 플래그, 확장 로딩 등)를 줄인다.
            # 완벽한 스텔스는 아니지만 비용 없이 넣을 수 있는 기본적인 처리다.
        ...
        # navigator.webdriver === true는 헤드리스 자동화의 가장 흔한 탐지 신호라
        # 페이지 스크립트가 실행되기 전에 지워둔다.
        ...
        try:
            await page.goto(cfg["url"], wait_until="domcontentloaded", timeout=45000)
            # 페이지 로드 직후 바로 데이터 요청이 안 오는 경우가 있어(느린 렌더링
            # 등, 반드시 봇 차단은 아님) 2초 -> 4초로 늘렸다. 그래도 안 잡히면
            # run_capture_sync()의 재시도 로직이 한 번 더 시도한다.
        ...
        try:
            ...
            async def _run():
                ...
                for sym in need_playwright_retry:
                    cfg = TARGETS[sym]
                    df = pd.DataFrame()
                    # 최대 2번 시도한다 — Barchart 응답이 매번 일관되지 않아서
                    # (봇 차단 가능성과 별개로, 단순 페이지 로딩 타이밍 문제로도
                    # 보임) 한 번 실패해도 곧바로 재시도하면 되는 경우가 많다.
                    for attempt in (1, 2):
                        try:
                            df = await capture_one(sym, cfg, async_playwright)
                        except Exception as e:
                            print(f"  [오류] {sym} Playwright 캡처 실패(시도 {attempt}/2): {e}")
                            df = pd.DataFrame()
                        if not df.empty:
                            break
                        if attempt == 1:
                            print(f"  [정보] {sym} 결과가 비어서 재시도합니다...")
                    out[sym] = df
                return out
    ...
    if symbols_with_data:
        try:
            ...
            async def _run_greeks():
                ...
                for sym in symbols_with_data:
                    ...
                    if not gcfg:
                        continue
                    df_g = pd.DataFrame()
                    for attempt in (1, 2):  # 옵션 페이지와 동일한 이유로 최대 2번 시도
                        try:
                            df_g = await capture_greeks_one(sym, gcfg, async_playwright)
                        except Exception as e:
                            print(f"  [오류] {sym} Greeks 캡처 실패(시도 {attempt}/2): {e}")
                            df_g = pd.DataFrame()
                        if not df_g.empty:
                            break
                        if attempt == 1:
                            print(f"  [정보] {sym} Greeks 결과가 비어서 재시도합니다...")
                    out[sym] = df_g
                return out
            ...
            for sym, df_greeks in greeks_results.items():
                ...
                # 방어적 처리: _merge_greeks가 그릭스 없이 df_main을 그대로 돌려주면
                # (그릭스 캡처 자체가 이번엔 빈 결과였던 경우) impliedVolatility
                # 컬럼이 아예 없을 수 있다. 여기서 죽으면 CSV 저장까지 통째로
                # 못 가니, 통계 출력 하나 때문에 전체 캡처가 실패하면 안 된다.
                if "impliedVolatility" in results[sym].columns:
