...
FUTURES_TICKER_ALIASES = {
    _ρ('TlE='): _ρ('TlE='), _ρ('TlE9Rg=='): _ρ('TlE='),
    _ρ('RVM='): _ρ('RVM='), _ρ('RVM9Rg=='): _ρ('RVM='),
}
FUTURES_CSV_DEFAULT = _ρ('YmFyY2hhcnRfb3B0aW9uc19jYXB0dXJlLmNzdg==')
FUTURES_MIN_IV_ROWS_DEFAULT = (10577 ^ _κ)
FUTURES_WALL_BANDWIDTH_PCT_DEFAULT = 0.01

TARGETS = {
    _ρ('TlE='): {_ρ('dXJs'): _ρ('aHR0cHM6Ly93d3cuYmFyY2hhcnQuY29tL2Z1dHVyZXMvcXVvdGVzL05RKjAvb3B0aW9ucz9tb25leW5lc3M9YWxsUm93cyZmdXR1cmVzT3B0aW9uc1ZpZXc9bWVyZ2Vk'), _ρ('bXVsdGlwbGllcg=='): 20.0},
    _ρ('RVM='): {_ρ('dXJs'): _ρ('aHR0cHM6Ly93d3cuYmFyY2hhcnQuY29tL2Z1dHVyZXMvcXVvdGVzL0VTKjAvb3B0aW9ucz9tb25leW5lc3M9YWxsUm93cyZmdXR1cmVzT3B0aW9uc1ZpZXc9bWVyZ2Vk'), _ρ('bXVsdGlwbGllcg=='): 50.0},
}
TARGETS_GREEKS = {
    _ρ('TlE='): {_ρ('dXJs'): _ρ('aHR0cHM6Ly93d3cuYmFyY2hhcnQuY29tL2Z1dHVyZXMvcXVvdGVzL05RKjAvdm9sYXRpbGl0eS1ncmVla3M/bW9uZXluZXNzPWFsbFJvd3MmZnV0dXJlc09wdGlvbnNWaWV3PW1lcmdlZA==')},
    _ρ('RVM='): {_ρ('dXJs'): _ρ('aHR0cHM6Ly93d3cuYmFyY2hhcnQuY29tL2Z1dHVyZXMvcXVvdGVzL0VTKjAvdm9sYXRpbGl0eS1ncmVla3M/bW9uZXluZXNzPWFsbFJvd3MmZnV0dXJlc09wdGlvbnNWaWV3PW1lcmdlZA==')},
}
MANUAL_OVERRIDE = {}
MAX_BODY_BYTES = (5005844 ^ _κ)
CAPTURE_API_URL = _ρ('aHR0cHM6Ly93d3cuYmFyY2hhcnQuY29tL3Byb3hpZXMvY29yZS1hcGkvdjEvcXVvdGVzL2dldA==')
CAPTURE_API_FIELDS = (
    _ρ('')
)
FUTURES_CSV_COLUMNS = [_ρ('c3ltYm9s'), _ρ('c3RyaWtl'), _ρ('dHlwZQ=='), _ρ('b3BlbkludGVyZXN0'), _ρ('aW1wbGllZFZvbGF0aWxpdHk='),
                        _ρ('ZGVsdGFfYmFyY2hhcnQ='), _ρ('Z2FtbWFfYmFyY2hhcnQ='), _ρ('dGhldGFfYmFyY2hhcnQ='), _ρ('dmVnYV9iYXJjaGFydA=='),
                        _ρ('ZXhwaXJhdGlvbg=='), _ρ('c3BvdA=='), _ρ('Y29udHJhY3RfbXVsdGlwbGllcg==')]
...
def solve_iv_roots_from_delta_b76(delta, F, K, T, r, opt_type: str, sigma_grid=None):
    if sigma_grid is None:
        sigma_grid = np.linspace(0.005, 3.0, (11748 ^ _κ))
    T_ = max(float(T), 1e-6)
...
def derive_effective_iv_futures(chain: pd.DataFrame, spot: float, r: float = 0.04) -> pd.DataFrame:
    ...
    for pos in range(n):
        ...
        if pd.isna(delta) or pd.isna(row.get(_ρ('VA=='))):
            continue
        roots = solve_iv_roots_from_delta_b76(float(delta), spot, float(row[_ρ('c3RyaWtl')]),
                                               float(row[_ρ('VA==')]), r, row[_ρ('dHlwZQ==')])
...
def compute_gex_curve(chain: pd.DataFrame, spot: float, price_range_pct: float = 0.15, n_points: int = (10541 ^ _κ)):
    ...
    for S in price_grid:
        ...
        call_gex = np.sum(call_gamma * calls[_ρ('b3BlbkludGVyZXN0')].values) * (10544 ^ _κ) * S ** 2 * 0.01
        put_gex = np.sum(put_gamma * puts[_ρ('b3BlbkludGVyZXN0')].values) * (10544 ^ _κ) * S ** 2 * 0.01
...
def compute_gex_curve_futures_bs(chain: pd.DataFrame, spot: float, multiplier: float, r: float,
                                  price_range_pct: float = 0.15, n_points: int = (10541 ^ _κ)):
...
def find_walls_futures_smoothed(chain: pd.DataFrame, spot: float, bandwidth_pct: float = 0.01,
    ...
    for opt_type, prefix in ((_ρ('Y2FsbA=='), _ρ('Y2FsbA==')), (_ρ('cHV0'), _ρ('cHV0'))):
        ...
        lo, hi = strikes.min() - (10583 ^ _κ) * bandwidth, strikes.max() + (10583 ^ _κ) * bandwidth
...
async def try_direct_api(page, referer_url: str, meta: dict, symbol: str) -> tuple:
    ...
    headers = {
        _ρ('QWNjZXB0'): _ρ('YXBwbGljYXRpb24vanNvbiwgdGV4dC9wbGFpbiwgKi8q'),
        _ρ('WC1SZXF1ZXN0ZWQtV2l0aA=='): _ρ('WE1MSHR0cFJlcXVlc3Q='),
        _ρ('UmVmZXJlcg=='): referer_url,
    }
    ...
    params = {
        _ρ('c3ltYm9s'): meta[_ρ('dGlja2Vy')], _ρ('bGlzdA=='): _ρ('ZnV0dXJlcy5vcHRpb25z'), _ρ('ZmllbGRz'): CAPTURE_API_FIELDS,
        _ρ('bWV0YQ=='): _ρ('ZmllbGQuc2hvcnROYW1lLGZpZWxkLmRlc2NyaXB0aW9uLGZpZWxkLnR5cGU='),
        _ρ('Z3JvdXBCeQ=='): _ρ('b3B0aW9uVHlwZQ=='), _ρ('b3JkZXJCeQ=='): _ρ('c3RyaWtl'), _ρ('b3JkZXJEaXI='): _ρ('YXNj'), _ρ('cmF3'): _ρ('MQ=='),
    }
    try:
        resp = await page.request.get(CAPTURE_API_URL, params=params, headers=headers, timeout=(26484 ^ _κ))
...
def _find_candidate_lists(obj, path=""):
    ...
    if isinstance(obj, dict):
        ...
        for i, x in enumerate(obj[:(10598 ^ _κ)]):
...
def _build_rows_from_records(records: list) -> list:
    ...
    has_prefixed = (
        any(re.search(_ρ('XmNhbGw='), k, re.I) for k in sample_keys)
        and any(re.search(_ρ('XnB1dA=='), k, re.I) for k in sample_keys)
    )
    rows = []
    if has_prefixed:
        for r in records:
...
def _build_rows_from_grouped(call_records: list, put_records: list) -> list:
    ...
    for records, opt_type in ((call_records, _ρ('Y2FsbA==')), (put_records, _ρ('cHV0'))):
        for r in records:
            ...
            if oi is not None:
                rows.append({
                    _ρ('c3RyaWtl'): strike, _ρ('dHlwZQ=='): opt_type, _ρ('b3BlbkludGVyZXN0'): oi,
                    _ρ('aW1wbGllZFZvbGF0aWxpdHk='): iv, _ρ('ZGVsdGFfYmFyY2hhcnQ='): delta, _ρ('Z2FtbWFfYmFyY2hhcnQ='): gamma,
                    _ρ('dGhldGFfYmFyY2hhcnQ='): theta, _ρ('dmVnYV9iYXJjaGFydA=='): vega,
                })
    return rows
...
def parse_from_json_candidates(captured: list) -> pd.DataFrame:
    ...
    for url, body in captured:
        ...
        for path, records in _find_candidate_lists(data):
            ...
            if score >= (10577 ^ _κ) and (flat_best is None or score > flat_best[0]):
...
def parse_from_html_table(html: str) -> pd.DataFrame:
    ...
    for i in range(len(target)):
        ...
        if not pd.isna(call_oi_c.iloc[i]):
            rows.append({_ρ('c3RyaWtl'): k, _ρ('dHlwZQ=='): _ρ('Y2FsbA=='), _ρ('b3BlbkludGVyZXN0'): call_oi_c.iloc[i],
                         _ρ('aW1wbGllZFZvbGF0aWxpdHk='): (call_iv_c.iloc[i] / 100.0) if call_iv_c is not None else None})
        if not pd.isna(put_oi_c.iloc[i]):
            rows.append({_ρ('c3RyaWtl'): k, _ρ('dHlwZQ=='): _ρ('cHV0'), _ρ('b3BlbkludGVyZXN0'): put_oi_c.iloc[i],
                         _ρ('aW1wbGllZFZvbGF0aWxpdHk='): (put_iv_c.iloc[i] / 100.0) if put_iv_c is not None else None})
    return pd.DataFrame(rows)
...
def capture_one_requests(symbol: str, cfg: dict) -> pd.DataFrame:
    ...
    s = requests.Session()
    s.headers.update({
        _ρ('VXNlci1BZ2VudA=='): (_ρ('TW96aWxsYS81LjAgKFdpbmRvd3MgTlQgMTAuMDsgV2luNjQ7IHg2NCkgQXBwbGVXZWJLaXQvNTM3LjM2IChLSFRNTCwgbGlrZSBHZWNrbykgQ2hyb21lLzEyNC4wIFNhZmFyaS81MzcuMzY=')),
        _ρ('QWNjZXB0LUxhbmd1YWdl'): _ρ('ZW4tVVMsZW47cT0wLjk='),
    })

    try:
        r = s.get(cfg[_ρ('dXJs')], timeout=(10560 ^ _κ))
        html = r.text
    except Exception as e:
    ...
    if ticker and csrf_token and xsrf_cookie:
        ...
        headers = {
            _ρ('QWNjZXB0'): _ρ('YXBwbGljYXRpb24vanNvbiwgdGV4dC9wbGFpbiwgKi8q'),
            _ρ('WC1SZXF1ZXN0ZWQtV2l0aA=='): _ρ('WE1MSHR0cFJlcXVlc3Q='),
            _ρ('UmVmZXJlcg=='): cfg[_ρ('dXJs')],
            _ρ('WC1DU1JGLVRPS0VO'): csrf_token,
            _ρ('WC1YU1JGLVRPS0VO'): xsrf_cookie,
        }
        params = {
            _ρ('c3ltYm9s'): ticker, _ρ('bGlzdA=='): _ρ('ZnV0dXJlcy5vcHRpb25z'), _ρ('ZmllbGRz'): CAPTURE_API_FIELDS,
            _ρ('bWV0YQ=='): _ρ('ZmllbGQuc2hvcnROYW1lLGZpZWxkLmRlc2NyaXB0aW9uLGZpZWxkLnR5cGU='),
            _ρ('Z3JvdXBCeQ=='): _ρ('b3B0aW9uVHlwZQ=='), _ρ('b3JkZXJCeQ=='): _ρ('c3RyaWtl'), _ρ('b3JkZXJEaXI='): _ρ('YXNj'), _ρ('cmF3'): _ρ('MQ=='),
        }
        try:
            r2 = s.get(CAPTURE_API_URL, params=params, headers=headers, timeout=(10560 ^ _κ))
            api_debug_text = f"status={r2.status_code}\n\n{r2.text}"
            if r2.status_code == (10652 ^ _κ) and r2.text:
...
async def capture_one(symbol: str, cfg: dict, async_playwright) -> pd.DataFrame:
    ...
    async with async_playwright() as p:
        ...
        page = await browser.new_page(user_agent=(
            _ρ('TW96aWxsYS81LjAgKFdpbmRvd3MgTlQgMTAuMDsgV2luNjQ7IHg2NCkgQXBwbGVXZWJLaXQvNTM3LjM2IChLSFRNTCwgbGlrZSBHZWNrbykgQ2hyb21lLzEyNC4wIFNhZmFyaS81MzcuMzY=')
        ))
        captured_json = []
        ...
        await page.goto(cfg[_ρ('dXJs')], wait_until=_ρ('ZG9tY29udGVudGxvYWRlZA=='), timeout=(34460 ^ _κ))
        await page.wait_for_timeout((11908 ^ _κ))
...
def _build_greeks_rows(records: list) -> list:
    ...
    for r in records:
        ...
        if iv is None and delta is None and gamma is None:
            continue
        rows.append({
            _ρ('c3RyaWtl'): strike, _ρ('dHlwZQ=='): opt_type, _ρ('aW1wbGllZFZvbGF0aWxpdHk='): iv,
            _ρ('ZGVsdGFfYmFyY2hhcnQ='): delta, _ρ('Z2FtbWFfYmFyY2hhcnQ='): gamma,
            _ρ('dGhldGFfYmFyY2hhcnQ='): theta, _ρ('dmVnYV9iYXJjaGFydA=='): vega,
        })
    return rows
    ...
    for records, opt_type in ((call_records, _ρ('Y2FsbA==')), (put_records, _ρ('cHV0'))):
        for r in records:
            ...
            if iv is None and delta is None and gamma is None:
                continue
            rows.append({
                _ρ('c3RyaWtl'): strike, _ρ('dHlwZQ=='): opt_type,
                _ρ('aW1wbGllZFZvbGF0aWxpdHk='): iv, _ρ('ZGVsdGFfYmFyY2hhcnQ='): delta, _ρ('Z2FtbWFfYmFyY2hhcnQ='): gamma,
                _ρ('dGhldGFfYmFyY2hhcnQ='): theta, _ρ('dmVnYV9iYXJjaGFydA=='): vega,
            })
    return rows
...
def _merge_greeks(df_main: pd.DataFrame, df_greeks: pd.DataFrame) -> pd.DataFrame:
    if df_greeks is None or df_greeks.empty:
        return df_main
    greek_cols = [_ρ('c3RyaWtl'), _ρ('dHlwZQ=='), _ρ('aW1wbGllZFZvbGF0aWxpdHk='), _ρ('ZGVsdGFfYmFyY2hhcnQ='),
                  _ρ('Z2FtbWFfYmFyY2hhcnQ='), _ρ('dGhldGFfYmFyY2hhcnQ='), _ρ('dmVnYV9iYXJjaGFydA==')]
    df_greeks = df_greeks[[c for c in greek_cols if c in df_greeks.columns]]
    merged = df_main.drop(
        columns=[c for c in greek_cols if c != _ρ('c3RyaWtl') and c != _ρ('dHlwZQ==') and c in df_main.columns]
    ).merge(df_greeks, on=[_ρ('c3RyaWtl'), _ρ('dHlwZQ==')], how=_ρ('bGVmdA=='))
    return merged
...
def run_futures_mode(args):
    ...
    if use_bs_mode:
        ...
        if n_uniq <= (10577 ^ _κ) or n_zero > 0:
...
def main():
    ...
    parser.add_argument(_ρ('LS10aWNrZXI='), default=_ρ('U1BZ'),
                         help=_ρ('7Ji17IWYIOycoOuPmeyEseydtCDsnojripQg7Yuw7LukICjquLDrs7g6IFNQWSkuIE5RL05RPUYvRVMvRVM9RuulvCDso7zrqbQgYmNkYiBDU1Yg6riw67CYIOyEoOusvOyYteyFmCDqsr3roZzroZwg7J6Q64+ZIOyghO2ZmOuQqeuLiOuLpC4='))
    parser.add_argument(_ρ('LS1tYXgtZGF5cw=='), type=int, default=(10617 ^ _κ), help=_ρ('7Y+s7ZWo7ZWgIOy1nOuMgCDrp4zquLDsnbzsiJggKOq4sOuzuDogNDXsnbwsIEVURiDqsr3roZzsl5DshJzrp4wg7IKs7JqpKQ=='))
    ...
    parser.add_argument(_ρ('LS1mdXR1cmVzLWNzdg=='), default=FUTURES_CSV_DEFAULT,
                         help=_ρ('Q1NWIOy6oeyymCDsvZTrk5wg6rKw6rO8IENTViDqsr3roZwgKE5RPUYvRVM9RiDsoITsmqksIOq4sOuzuDogYmFyY2hhcnRfb3B0aW9uc19jYXB0dXJlLmNzdik='))
    parser.add_argument(_ρ('LS1mdXR1cmVzLW1pbi1pdi1yb3dz'), type=int, default=FUTURES_MIN_IV_ROWS_DEFAULT,
                         help=_ρ('7L2cL+2SiyDqsIHqsIEg7J20IOqwnOyImCDsnbTsg4Eg7Jyg7ZqoIElW6rCAIOyeiOyWtOyVvCBCUyDsiqTsupQg66qo65OcIOyCrOyaqSAoTlE9Ri9FUz1GIOyghOyaqSwg6riw67O4IDUp'))
    parser.add_argument(_ρ('LS1mdXR1cmVzLXdhbGwtYmFuZHdpZHRoLXBjdA=='), type=float, default=FUTURES_WALL_BANDWIDTH_PCT_DEFAULT,
                         help=_ρ('7Iqk66y065SpIOy9nOyblC/tkovsm5Tsmqkg6rCA7Jqw7Iuc7JWIIOy7pOuEkCDtj60sIHNwb3Qg64yA67mEIOu5hOycqCAoTlE9Ri9FUz1GIOyghOyaqSwg6riw67O4IDAuMDEp'))
    parser.add_argument(_ρ('LS1yZWNhcHR1cmU='), action=_ρ('c3RvcmVfdHJ1ZQ=='),
                         help=_ρ('Q1NW6rCAIOydtOuvuCDsnojslrTrj4QgYmNkYuyXkOyEnCDqsJXsoJzroZwg7IOI66GcIOy6oeyymCAoTlE9Ri9FUz1GIOyghOyaqSk='))
    parser.add_argument(_ρ('LS1jYXB0dXJlLWJvdGg='), action=_ρ('c3RvcmVfdHJ1ZQ=='),
                         help=_ρ('7Lqh7LKY6rCAIO2VhOyalO2VoCDrlYwg7JqU7LKt7ZWcIOyLrOuzvOunjOydtCDslYTri4jrnbwgTlEvRVMg65GYIOuLpCDtlZwg67KI7JeQIOy6oeyymCAoTlE9Ri9FUz1GIOyghOyaqSk='))
    parser.add_argument(_ρ('LS1uby1jYXB0dXJl'), action=_ρ('c3RvcmVfdHJ1ZQ=='),
                         help=_ρ('Q1NW6rCAIOyXhuyWtOuPhCDsnpDrj5kg7Lqh7LKY66W8IOyLnOuPhO2VmOyngCDslYrqs6Ag67CU66GcIOyXkOufrCAoTlE9Ri9FUz1GIOyghOyaqSk='))
    parser.add_argument(_ρ('LS1kZWJ1Zw=='), action=_ρ('c3RvcmVfdHJ1ZQ=='),
                         help=_ρ('W+uUlOuyhOq3uF0g7YOc6re4IOu2meydgCDsg4HshLgg7Lqh7LKYL+2MjOyLsSDroZzqt7jrpbwg7Lac66ClICjquLDrs7jsnYAg6rq87KeQKQ=='))
    ...
    print(walls_by_exp.head((10577 ^ _κ)).to_string(index=False))
